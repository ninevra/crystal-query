...
export function operatorParser(...operators) {
  ...
  // regexes (test how?)
  const individualParsers = operators.map(operator => parsimmon.string(operator));
...
export class Parser {
  constructor({
    ...
    this.language = parsimmon.createLanguage({
      ...
      parenthetical: (l) => l.expression.wrap(l.lparen, l.rparen).node('Parenthetical'),
      ...
      expression: (l) => parsimmon.alt(
        l.list,
        parsimmon.optWhitespace.node('Nil')
      ),
