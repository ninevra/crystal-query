...
test('language.term recognizes', t => {
  ...
  t.like(term.parse('>2'), {status: true, value: {
    name: 'Term', value: ['', '>', '2']
  }});
  t.like(term.parse('foo:"bar"'), {status: true, value: {name: 'Term', value: ['foo', ':', 'bar']}});
});
...
test('language.negation recognizes', t => {
  ...
  t.like(negation.parse('not"foo"'), {status: true, value: {name: 'Not', value: {name: 'Term', value: ['', '', 'foo']}}});
});
...
test('language.conjunction recognizes', t => {
  ...
  t.like(result.value.value[0], {name: 'Not', value: {name: 'Term', value: ['', '', 'foo']}});
  t.like(result.value.value[1], {name: 'Not', value: {name: 'Term', value: ['', '', 'bar']}});
});
...
test('language.disjunction recognizes', t => {
  ...
  t.like(result, {status: true, value: {name: 'Or'}});
  t.like(result.value.value[0], {name: 'Not', value: {name: 'Term', value: ['', '', 'a']}});
...
test('language.list recognizes', t => {
  ...
  t.like(result.value.value[0].value[0], {name: 'Term', value: ['', '', 'a']});
  t.like(result.value.value[0].value[1], {name: 'Term', value: ['', '', 'b']});
...
test('language.parenthetical', t => {
  ...
  t.like(result, {status: true, value: {name: 'Parenthetical', value: {name: 'And'}}});
  result = conjunction.parse('(a or b) and c');
  t.like(result, {status: true, value: {name: 'And'}});
  t.like(result.value.value[0], {name: 'Parenthetical', value: {name: 'Or'}});
  t.like(result.value.value[1], {name: 'Term', value: ['', '', 'c']});
});
...
test('empty expressions', t => {
  ...
  t.like(result, {status: true, value: {name: 'Not', value: {name: 'Parenthetical'}}});
});
